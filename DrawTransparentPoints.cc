...

{
    ...
    // engEvalString(ep_matrix, "imagesc(catheter_potential); ");
    ...
    for (int j=0; j< curmesh->data->numframes; j++)
    {
        ...
        for (int i=0; i< atria_num; i++)
        {
           ...
           //std::cout<<"mfsEGM[i+j*curmesh->data->numframes];  " <<mfsEGM[i+j*curmesh->data->numframes]<<"   "<<i<<"    "<<j<<std::endl;
        }
    }


    //cursurf->MFSvals=mfs_final;

   // engEvalString(ep_matrix, "figure; ");
   // engEvalString(ep_matrix, "imagesc(mfsEGM); ");
}
