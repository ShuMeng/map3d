...
//      if (loop == 0) {
//        colorval = minactivation;
//      }
//      else if (loop == contvals.size() - 2) {
//        colorval = maxactivation;
//      }
//      else {
...
//      getContColor(colorval, minactivation, maxactivation, *(this->map), color, mesh->invert);
//      glColor3ubv(color);


//      //horiz
//      if (!orientation) {
//        glVertex2d(left + (lastval - minactivation) / (maxactivation - minactivation) * size, vsize);
//        glVertex2d(left + (lastval - minactivation) / (maxactivation - minactivation) * size, bottom);
//        glVertex2d(left + (nextval - minactivation) / (maxactivation - minactivation) * size, bottom);
//        glVertex2d(left + (nextval - minactivation) / (maxactivation - minactivation) * size, vsize);
//      }
//      else {
//        glVertex2d(left, bottom + (lastval - minactivation) / (maxactivation - minactivation) * size);
//        glVertex2d(hsize, bottom + (lastval - minactivation) / (maxactivation - minactivation) * size);  // -.85 to .4
//        glVertex2d(hsize, bottom + (nextval - minactivation) / (maxactivation - minactivation) * size);  // -.85 to .4
//        glVertex2d(left, bottom + (nextval - minactivation) / (maxactivation - minactivation) * size);
//      }
//    }
//    glEnd();
  }
  ...
  for (loop = 0; loop < (unsigned) contvals.size(); loop++) {
    ...
    //horizontal
    if (!orientation) {
      position[0] = left - getFontWidth(mesh->gpriv->small_font, "XXX") + ((nextval - minactivation) / (maxactivation - minactivation) * size);
